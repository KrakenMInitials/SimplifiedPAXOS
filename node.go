...
func (n *Node) ProcessPrepareRequest(args *shared.PrepareRequest, reply *shared.PrepareResponse) error { //phase 1
	...
	if (n.highestPrpslNum != 0) && (args.PrpslNum <= n.highestPrpslNum) { //true reject
		...
		fmt.Println("Ph.1: Acceptor rejected and states are ", n.highestPrpslNum, *n.knownVal)
	...
	if n.knownVal == nil {
		...
		fmt.Println("Ph.1: Acceptor accepted and states are ", n.highestPrpslNum, *n.knownVal)

		reply.ConsensusRoundID = args.ConsensusRoundID
...
func (n *Node) ProcessAcceptRequest(args *shared.AcceptRequest, reply *shared.AcceptResponse) error {
	...
	if n.highestPrpslNum == args.PrpslNum { //value accepted and sent to distinguished learner
		n.complete = true
		fmt.Println("Ph.2: Acceptor accepted with ", n.ConsensusRound, n.highestPrpslNum, *n.knownVal)
		...
		fmt.Println("Ph.2: Acceptor rejected with ", n.ConsensusRound, n.highestPrpslNum, *n.knownVal)
...
RetryProposal:
	for i := 1; ; i += 1 {
		time.Sleep(time.Duration(rand.Intn(5)+1) * time.Second) // random timeout for infinite prepare request loop prevention
		//
		// PHASE 1
		//

		if args.ConsensusRoundID < n.ConsensusRound {
			fmt.Println("Node recieved new consensus round; stopped proposing for", args.ConsensusRoundID)
			return nil
		}
		...
		fmt.Println("Round", n.ConsensusRound, ":Proposing #", n.proposedPrpslNum, " ", v)
		...
		for peer_id, client := range n.PeerClients {
			go func(c *rpc.Client) {
				...
				case <-call.Done:
					...
					if prepare_response.ConsensusRoundID == consensus_round {
						...
						fmt.Println("Node recieved older consensus round; stopped proposing for", args.ConsensusRoundID)
						isOlderRound = true
						return
					}
		...
		for {
			...
			if failure_count == len(shared.Known_acceptors) {
				continue RetryProposal
			} //all failed ->
			if success_count > len(shared.Known_acceptors)/2 { //majority accepted
				break //continue phase 2
			}
			if failure_count+success_count == len(shared.Known_acceptors) {
				continue RetryProposal
			} //didnt hit majority
		}
